...
interface EditTextModalProps {
  element: TextElement;
  onClose: () => void;
  onSave: (_updated: TextElement) => void;
}

function EditTextModal({ element, onClose, onSave }: EditTextModalProps) {
  const [content, setContent] = useState(element.content);
  const [fontSize, setFontSize] = useState(element.fontSize);
  const [colour, setColour] = useState(element.colour);
  ...
  const handleSave = () => {
    ...
    onSave({ ...element, content, fontSize, colour, fontFamily, width, height, x, y });
  };

  return (
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <h2 className={styles.heading}>Edit Text Block</h2>
